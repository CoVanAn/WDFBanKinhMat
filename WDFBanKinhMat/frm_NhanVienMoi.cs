    ...
    public partial class frm_NhanVienMoi : Form
    {
        ...
        private void txtMa_TextChanged(object sender, EventArgs e)
        {
            if (txtTen.Text != "" || txtMa.Text != "")
            {
                ...
                {
                    string Select = "select MaNV, TenNV, TenCV from NhanVien join CongViec on NhanVien.MaCV = CongViec.MaCV where TenNV like N'%" + txtTen.Text + "%' and MaNV like '%" + txtMa.Text + "%' and TenCV like N'" + cbxCongViec.Text + "'";
                    DataTable dtSl = data.ReadData(Select);
                    dgvThongTinNV.DataSource = dtSl;
                }
                else
                {
                    string Select = "select MaNV, TenNV, TenCV from NhanVien join CongViec on NhanVien.MaCV = CongViec.MaCV where TenNV like N'%" + txtTen.Text + "%' and MaNV like '%" + txtMa.Text + "%'";
                    DataTable dtSl = data.ReadData(Select);
                    dgvThongTinNV.DataSource = dtSl;
                }
            }
            else { cbxCongViec_SelectedIndexChanged(sender, e); }
        }
            ...
            */
            ...
            else 
            {
                if (txtMa.Text == "" && txtTen.Text == "")
                {
                    ...
                    {
                        string select = "select MaNV, TenNV, TenCV from NhanVien join CongViec on NhanVien.MaCV = CongViec.MaCV where TenCV like N'" + cbxCongViec.Text + "'";
                        DataTable dataTable = data.ReadData(select);
                        DataTable dtSl = data.ReadData(select);
                        dgvThongTinNV.DataSource = dtSl;
                    }
                    else
